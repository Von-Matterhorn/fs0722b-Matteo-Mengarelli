...
function conv(){
  let arr = ['Matteo','Mengarelli'];
  let i;
  let lunghezza = [];
  for(i = 0; i < arr.length; i++){
    lunghezza[i] = arr[i].length;
  }
...
function anno(){
  ...
  console.log('Il film più vecchio è ' + nome + ' del ' + anno)
}
anno()
...
function num(){
  ...
  console.log('Il numero di film è ' + numero);
}
num()
...
function id(){
  ...
  for(i = 0; i < movies.length; i++){
    if(id == movies[i].imdbID){
      console.log('Il film selezionato è ' + movies[i].Title)
    }
  }
}
id()
...
document.getElementById('bottone').addEventListener('click', function find(){
  let parola = document.getElementById("parola").value;
  console.log(parola)
  let i;
  for(i = 0; i < movies.length; i++){
    if(movies[i].Title.includes(parola) || movies[i].Year.includes(parola)){
      console.log('Hai selezionato ' + movies[i].Title)
    }
  }
})
